...
8. **Forward compatibility** — will this design need significant rework for the next 2–3 features on the product roadmap? If yes, is that tradeoff explicitly accepted?
...
- [ ] For Tier 3+ or security-sensitive changes: threat model exists (`docs/02-design/technical/threat-model-*.md`) and all Critical/High findings have mitigations in the design. If missing, run `/review-security --phase design` first.

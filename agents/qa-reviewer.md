---
name: qa-reviewer
description: QA reviewer agent. Reviews test plans and test evidence against acceptance criteria and the incident registry. Use after the TDD cycle is complete to verify test coverage is sufficient before the Architect Code Review step. Write access limited to tests/ and docs/ (test registry updates only).
---

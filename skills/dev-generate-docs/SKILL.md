...
2. **Create `docs/02-design/technical/hld/<feature-name>.md`** using the template at `${CLAUDE_PLUGIN_ROOT}/shared/templates/hld-template.md`. The document must contain:
   - Executive summary
...
2. **For each component**, create `docs/02-design/technical/lld/<category>/<component-name>.md` using `${CLAUDE_PLUGIN_ROOT}/shared/templates/lld-component-template.md`. Each file must include:
   - Overview + purpose
...
2. **Generate `docs/system-manifest.yaml`** with ALL sections from `${CLAUDE_PLUGIN_ROOT}/shared/templates/system-manifest.schema.yaml`:
...
1. **For each major system area**, generate an HLD using `${CLAUDE_PLUGIN_ROOT}/shared/templates/hld-template.md`:
...
1. **For each domain in the manifest**, generate LLDs using `${CLAUDE_PLUGIN_ROOT}/shared/templates/lld-component-template.md`:
...
2. **For each detected decision**, generate a forensic ADR using `${CLAUDE_PLUGIN_ROOT}/shared/templates/adr-template.md`:
...
1. **Generate `CLAUDE.md`** from the template at
   ...
   **If `CLAUDE.md` already exists, do not overwrite it.** It is a file teams edit, and it carries
   ...
   written by onboarding, and the `HITL:PREFS:BEGIN`/`HITL:PREFS:END` block written by
   `/hitl:dev-preferences`. Show what you would add and let them merge it, or fill only the sections
   that are still empty. Never write over their own rules.
...
6. **Create registry stubs** if they don't exist:
   ...
   - **Test registry** (`docs/03-engineering/testing/test-registry.yaml`): generate from the test files discovered in Phase R1 using the schema from `${CLAUDE_PLUGIN_ROOT}/shared/templates/test-registry-template.yaml`. One entry per test file; populate `id`, `name`, `domain`, `file`, and `type` from what is discoverable; set `risk: DRAFT` and `origin: tdd`; leave `incident_ref: null` for the architect to classify.
   - **Incident registry** (`docs/04-operations/incident-registry.yaml`): create an empty stub from `${CLAUDE_PLUGIN_ROOT}/shared/templates/incident-registry-template.yaml` — header and schema structure only, no fabricated incidents.
...
8. **Identify training plan candidates** — scan the codebase for capabilities that would benefit from a training plan:
   ...
   - For each candidate, create a stub at `docs/03-engineering/training/<name>.md` using `${CLAUDE_PLUGIN_ROOT}/shared/templates/training-plan-template.md` with module outlines and reading lists pointing to the just-generated LLDs

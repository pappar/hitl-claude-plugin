...
1. **What is the delivery surface?** Web UI, mobile (iOS/Android/responsive web), API/backend only, agentic workflow (AI agent with no direct UI), internal/ops tool, or a combination? The answer gates which phases apply:
   ...
   **Compound-surface probe (agentic only — routes to the compound-agentic surface, EPIC #10).** Only if the surface is agentic, ask two cheap questions before designing: **(a)** how many distinct components (services / agents / stores)? **(b)** does any component call, hand off to, or message another? **≥2 components AND ≥1 inter-component edge → the compound-agentic surface** (a graph of deterministic services + simple/deep agents with sync/async/event edges): **run `hitl:agentic-intake` first** — it elicits the shape/risks, recommends a right-sized set of controls, records the decisions, and hands off a neutral `agentic-design-handoff.yaml`. **Then** a human authors the `system-manifest.yaml` from that handoff (Phase 4's compound track), which `ci/manifest-agentic` gates. A single agent → the existing single-agent agentic path. A 2-service deterministic app that answered "not agentic" never reaches this probe. *(The Advisor recommends and records; it authors no manifest field — #10 needs no input from it and validates the human-authored manifest.)*
